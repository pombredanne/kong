  ...
  describe("/cache/", function()
    ...
    describe("GET", function()
      ...
      it("[SUCCESS] should get the value of a cache item", function()
        ...
        -- Retrieving cache
        local response, status = http_client.get(BASE_URL.."ALL_APIS_BY_DIC")
        assert.equal(200, status)
    ...
    describe("DELETE", function()
      ...
      it("[SUCCESS] should invalidate an entity", function()
        ...
        -- Delete
        local _, status = http_client.delete(BASE_URL.."ALL_APIS_BY_DIC")
        assert.equal(200, status)
        
        -- Make sure it doesn't exist
        local _, status = http_client.get(BASE_URL.."ALL_APIS_BY_DIC")
        assert.equal(404, status)
      end)
